...
import '../../../../utils/extensions/adaptive_type.dart';
...
class PropertyModel {
  ...
  final List<dynamic>? extrafacilities;
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'parameters': parameters != null
          ? parameters!.map((param) => param.toJson()).toList()
          : [],
      'furnished': furnished,
      'assignedOutdoorFacility': assignedOutdoorFacility != null
          ? assignedOutdoorFacility!.map((facility) => facility.toJson()).toList()
          : [],
  ...
  factory PropertyModel.fromSnapshot(DocumentSnapshot snapshot) {
    final data = snapshot.data() as Map<String, dynamic>;

    return PropertyModel(
      id: snapshot.id,
      properyType: data['propertyType'] ?? '',
      addedBy: data['addedBy'] ?? '',
      titleImage: data['titleImage'] ?? '',
      gallery: data['gallery'] != null
          ? (data['gallery'] as List<dynamic>) // Ensure data['gallery'] is treated as a list
          .map((item) => Gallery.fromJson(item as Map<String, dynamic>)) // Ensure item is a Map
          .toList() // Convert the Iterable to a List<Gallery>
          : <Gallery>[], // Use a typed empty list
      title: data['title'] ?? '',
      price: data['price'] ?? '',
      ...
      latitude: data['latitude'] ?? '',
       parameters: data['parameters'] != null
           ? List<Parameter>.from(
           data['parameters'].map((param) => Parameter.fromJson(param)))
           : [],
      furnished: data['furnished'] ?? '',
      assignedOutdoorFacility: data['assignedOutdoorFacility'] != null
          ? List<AssignedOutdoorFacility>.from(data['assignedOutdoorFacility']
          .map((facility) => AssignedOutdoorFacility.fromJson(facility)))
           : [],
       extrafacilities: data['extrafacilities'] != null
           ? List<dynamic>.from(data['extrafacilities'])
           : [],
      searchableKeywords: data['searchableKeywords'] != null
          ? List<String>.from(data['searchableKeywords'])
          : [],
      postCreated: data['postCreated'] != null
          ? DateTime.parse(data['postCreated'])
          : DateTime.now(), // Default to now if not provided
    );
  }
  factory PropertyModel.fromQuerySnapshot(QueryDocumentSnapshot snapshot) {
    final data = snapshot.data() as Map<String, dynamic>;
  ...
  @override
  String toString() {
    return 'PropertyModel(id: $id,rentduration:$rentduration , title: $title,assigned_facilities:[$assignedOutdoorFacility]  advertisment:$advertisment, price: $price, category: $category,, builtUpArea: $builtUpArea, plotArea: $plotArea, hectaArea: $hectaArea, acre: $acre, houseType: $houseType, furnished: $furnished, unitType: $unitType, description: $description, address: $address, clientAddress: $clientAddress, properyType: $properyType, titleImage: $titleImage, title_image_hash: $titleimagehash, postCreated: $postCreated, gallery: $gallery, totalView: $totalView, status: $status, region: $region, city: $city, country: $country, addedBy: $addedBy, inquiry: $inquiry, promoted: $promoted, isFavourite: $isFavourite, isInterested: $isInterested, favouriteUsers: $favouriteUsers, interestedUsers: $interestedUsers, totalInterestedUsers: $totalInterestedUsers, totalFavouriteUsers: $totalFavouriteUsers, parameters: $parameters, latitude: $latitude, longitude: $longitude, threeD_image: $threeDImage, video: $video)';
  }
}
...
class Parameter {
  ...
  factory Parameter.fromJson(Map<String, dynamic> json) {
    return Parameter(
      ...
      typeValues: json['type_values'] != null ? List<String>.from(json['type_values']) : [],
      image: json['image'],
      value: json['value'],
    );
  }
...
class Gallery {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'imageUrl': imageUrl,
      'isVideo': isVideo,
      // imageFile MUST NOT be included — can't serialize File, plus you don't need it in Firestore
    };
  }
